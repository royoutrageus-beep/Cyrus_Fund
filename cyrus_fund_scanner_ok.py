            ...
            if n_fetched == 0:
                ...
                for i,ticker_yf in enumerate(tickers):
                    ...
                    try:
                        ...
                        # Turnover filter (lebih dulu): kalau turnover kecil, tetap buang.
                        # RVOL bisa NaN saat AvgVol belum matang (rolling 20), jadi jangan terlalu ketat.
                        if turnover < min_turn:
                            continue

                        # RVOL NaN/invalid → fallback pakai volume vs MA10 untuk mengurangi false-negative.
                        rvol_ok = True
                        if rvol is None or np.isnan(rvol) or rvol <= 0:
                            try:
                                vol_ma10 = float(df['Volume'].rolling(10).mean().iloc[-1])
                                rvol = float(r['Volume']) / vol_ma10 if vol_ma10 and vol_ma10 > 0 else np.nan
                            except:
                                rvol_ok = False

                        # Kalau masih invalid, lewati RVOL threshold (supaya screening tidak kosong).
                        if rvol_ok and (rvol is not None) and (not np.isnan(rvol)):
                            if rvol < vol_thresh:
                                continue
